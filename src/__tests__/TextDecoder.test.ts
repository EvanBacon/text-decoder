...
describe("TextDecoder", () => {
  ...
  it("Encoding names are case insensitive", () => {
    ...
    encodings.forEach((test) => {
      expect(new TextDecoder(test.label.toLowerCase()).encoding).toBe(
        test.encoding
      );
      expect(new TextDecoder(test.label.toUpperCase()).encoding).toBe(
        test.encoding
      );
    });
  });
    ...
    const cases = [
      {
        encoding: "utf-8",
        encoded: [
          0, 49, 50, 51, 65, 66, 67, 97, 98, 99, 194, 128, 195, 191, 196, 128,
          225, 128, 128, 239, 191, 189, 240, 144, 128, 128, 244, 143, 191, 191,
        ],
      },
    ];
  ...
  describe("Supersets of ASCII decode ASCII correctly", () => {
    it.each(["utf-8"])(
      "should decode ASCII correctly for encoding: %s",
      (encoding) => {
        let string = "";
        const bytes = [];
        for (let i = 0; i < 128; ++i) {
    ...
    const cases = [
      {
        encoding: "utf-8",
        expected: [
          0x7a, 0xc2, 0xa2, 0xe6, 0xb0, 0xb4, 0xf0, 0x9d, 0x84, 0x9e, 0xf4,
          0x8f, 0xbf, 0xbd,
        ],
      },
    ];
